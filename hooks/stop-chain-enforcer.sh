...
if zensu_hook_enabled selfReview && [ "$CODE_REVIEW_DONE" = "true" ]; then
  REASON="STOP intercepted by zensu chain-enforcer. The code-reviewer chain has converged (codeReviewDone) but the terminal self-review stage has not run. Your VERY NEXT action MUST be to invoke the /zensu-self-review skill — it performs a final critical self-reflection over this session's changes, takes at most one fix round under the still-active TDD phase-gate, and OWNS the chain terminus (it runs 'PLUGIN_ROOT=\"\$(bash \"\$HOME/.kiro/zensu/hooks/lib/resolve-plugin-root.sh\" 1)\" && bash \"\$PLUGIN_ROOT/hooks/lib/zensu-log.sh\" --chain-done'). Do NOT end your turn, do NOT re-run the reviewer agent, and do NOT run --chain-done yourself — let /zensu-self-review finalize the chain."
else
  REASON="STOP intercepted by zensu chain-enforcer. A main-thread TDD session finished implementation (or a fix round) but the zensu-code-reviewer chain has not completed. Your VERY NEXT action MUST be to use the subagent tool with agent 'zensu-code-reviewer', passing as the prompt a one-paragraph summary of what was implemented/fixed PLUS the list of changed files (use 'git diff --name-only HEAD' to enumerate). Do NOT end your turn, and do NOT fix anything inline first — review the report, fix any Critical/Important findings under the gate, then re-run the reviewer until it PASSes or you hit the round cap. Only valid exception: if implementation produced ZERO file changes, run 'PLUGIN_ROOT=\"\$(bash \"\$HOME/.kiro/zensu/hooks/lib/resolve-plugin-root.sh\" 1)\" && bash \"\$PLUGIN_ROOT/hooks/lib/zensu-log.sh\" --chain-done' and then stop."
fi
