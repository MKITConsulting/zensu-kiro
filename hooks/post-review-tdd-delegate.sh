...
if [ "$SELF_REVIEW_ON" = "1" ]; then
  CLOSE_PASS="run 'PLUGIN_ROOT=\"\$(bash \"\$HOME/.kiro/zensu/hooks/lib/resolve-plugin-root.sh\" 1)\" && bash \"\$PLUGIN_ROOT/hooks/lib/zensu-log.sh\" --code-review-done', then your VERY NEXT action must be to invoke the /zensu-self-review skill — the terminal self-review stage that owns the chain terminus and renders the final CHAIN-END SUMMARY. Do NOT close the chain yourself, do NOT render the summary here, and do NOT end your turn — self-review finalizes the chain."
  TAIL_DIRECTIVE=""
else
  CLOSE_PASS="close the review chain by running 'PLUGIN_ROOT=\"\$(bash \"\$HOME/.kiro/zensu/hooks/lib/resolve-plugin-root.sh\" 1)\" && bash \"\$PLUGIN_ROOT/hooks/lib/zensu-log.sh\" --chain-done', then stop."
