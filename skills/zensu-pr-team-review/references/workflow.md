...
- **Reference the persona template in the prompt**: don't inline the full template — the agent can `Read` this skill's own `references/reviewer-personas.md` (installed at `~/.kiro/skills/zensu-pr-team-review/references/reviewer-personas.md`, workspace scope: `.kiro/skills/...`) if it needs the schema.
