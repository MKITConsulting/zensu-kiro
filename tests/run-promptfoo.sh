...
# costs credits.
#
#   bash tests/run-promptfoo.sh diagnostics   # risk-register suite (D1-D4, D6)
...
#   PROMPTFOO_VERSION     exact pin (default 0.121.12)
set -u
...
PF="promptfoo@${PROMPTFOO_VERSION:-0.121.12}"
...
cleanup() { rm -f "${KIRO_HOME:-$HOME/.kiro}/agents/zensu-dump.json" 2>/dev/null || true; }
trap cleanup EXIT
...
cd "$PF_DIR"
npx -y "$PF" eval -c "$CONFIG" --no-cache -j 1 --output "$OUT" ${FILTER_ARGS[@]+"${FILTER_ARGS[@]}"}
RC=$?
echo "results: $OUT"
exit "$RC"
