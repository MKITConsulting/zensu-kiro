...
**Vanilla implementation mode (`hooks.tddImplementation:false`).** At `--tdd-begin` the config is read ONCE and frozen into the state file's `vanilla` flag (the freeze is written BEFORE the `active` flag; a freeze-write failure aborts activation); the command echoes the effective mode (`mode: strict` / `mode: vanilla`) so the skill knows which deltas to apply. While `vanilla` is `true` the gate passes the edit through after the `active` check, the path classification, and the session-state deny — the whole phase matrix above is bypassed, no phase markers are required, and tests are at the agent's discretion. The gate reads ONLY the state flag, never live config: flipping the config mid-session can neither un-gate a strict session nor re-arm a vanilla one (whose phase stays `UNINITIALIZED` and would otherwise deny everything). In BOTH modes the gate denies edit-tool writes touching the session-state files (`.zensu/state/` and the rounds-counter dir, normalized and realpath-resolved) while a session is active — unless the user bypasses the gate entirely with `ZENSU_TDD_GATE=off`; state flags change only through `zensu-log.sh` (shell-tool writes are outside this hook's mediation, per the trust boundary above). One more mode-independent outcome exists: when the gate's internal path-classification pass fails, the call is denied with `path classification unavailable` (fail-closed for the state protection; the deny names the `ZENSU_TDD_GATE=off` escape hatch). This fail-closed guarantee holds only while node can run at all — with node absent or failing before payload extraction the gate (like every hook in this plugin) is a silent pass-through. Still enforced in vanilla mode: the shell witness, the Phase 5/6 evidence audits (build, coverage, witness cross-check), the review fan-out → `zensu-code-reviewer` → auto-fix loop → `/zensu-self-review`, and the Stop-hook chain guarantee. `--tdd-reset` clears the flag; a later `--tdd-begin` re-freezes it from the then-current config.
